-- from TPIL: https://leanprover.github.io/theorem_proving_in_lean4/axioms_and_computation.html
...
instance [de : DecidableEq α] {a₁ a₂ b₁ b₂ : α} : Decidable (@eqv α (a₁,a₂) (b₁,b₂)) :=
  instDecidableOr (dp := instDecidableAnd (dp := de _ _) (dq := de _ _))
                  (dq := instDecidableAnd (dp := de _ _) (dq := de _ _))
...
theorem ex1 {x y z w : Nat} : {{x,y},{z,w}} = {{w,z},{x,y}} := by
   have h := UProd.mk_eq_mk {x,y} {z,w}
   rw [h, UProd.mk_eq_mk w z]

--instance {α} : HasEquiv (α × α) := instHasEquivOfSetoid
--theorem ex1_exact := Quotient.exact ex1
#check Quotient.exact ex1
#check fun a b => Quotient.exact (UProd.mk_eq_mk a b)

/-
theorem UProd.pair_mk_congr : a₁ ~ a₂ → b₁ ~ b₂ → (a₁,b₁) ~ (a₂,b₂) := by
  sorry -- false!

theorem ex_false : ((1,2),(3,4)) ~ ((1,2),(4,3)) := UProd.pair_mk_congr (Or.inl ⟨rfl, rfl⟩)
  (Or.inr ⟨rfl, rfl⟩)

theorem ex_false' : ¬ ((1,2),(3,4)) ~ ((1,2),(4,3)) := by decide

example : False := ex_false' ex_false
-/
